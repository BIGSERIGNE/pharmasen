...
export default function BottomNav({ profile }: BottomNavProps) {
  ...
  if (profile?.role === 'pharmacist') {
    const pharmaLinks = [
      { to: '/pharma/dashboard', icon: LayoutDashboard, label: 'Dashboard', key: 'dash' },
      { to: '/pharma/stock', icon: Package, label: 'Stock', key: 'stock' },
      { to: '/pharma/dashboard', icon: ClipboardList, label: 'Commandes', key: 'orders' },
      { to: '/pharma/profile', icon: User, label: 'Profil', key: 'profile' },
    ];
    return (
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 flex">
        {pharmaLinks.map(({ to, icon: Icon, label, key }) => (
          <Link
            key={key}
            to={to}
            className={cn(
              'flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-xs transition-colors',
              isActive(to) && key !== 'orders' ? 'text-primary-600' : 'text-gray-500',
            )}
          >
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 flex">
      <Link
        ...
        className={cn(
          'flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-xs transition-colors',
          isActive('/home') ? 'text-primary-600' : 'text-gray-500',
        )}
      >
      ...
      <Link
        ...
        className={cn(
          'flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-xs transition-colors',
          isActive('/notifications') ? 'text-primary-600' : 'text-gray-500',
        )}
      >
